...
if __name__ == "__main__":
    ...
    dataWriter = DataWriter(outputPath=f"{config.thLaplacePath}{caseName}/")
    print(dataWriter.outputPath)
    for tIdx in timeArange:
        print(f"========== {tIdx:06d} ==========")
        buoyancy = np.array(nc.Dataset(config.thBuoyancyPath + f"{caseName}/buoyancy-{tIdx:06d}.nc")["buoyancy"][0])
