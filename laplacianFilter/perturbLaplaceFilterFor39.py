...
if __name__ == "__main__":
    ...
    dataWriter = DataWriter(outputPath=f"{config.perturbLaplacePath}{caseName}/gaussian-{numFrameX}/")

    for tIdx in timeArange:
        print(f"========== {tIdx:06d} ==========")
        buoyancy = np.array(nc.Dataset(config.perturbBuoyancyPath + f"{caseName}/gaussian-{numFrameX}/buoyancy-{tIdx:06d}.nc")["buoyancy"][0])
