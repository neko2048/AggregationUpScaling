...
def getGaussianConvolve1D(data, kernel, method='fft'):
    from scipy.ndimage import convolve1d
    ny, nx = data.shape[1], data.shape[2]
    convData = np.zeros(shape=data.shape)

    for i in range(data.shape[0]):
        convData[i] = convolve1d(input=data[i, 0], weights=kernel[0], mode="wrap")
    return convData
...
def getTildeAnomalyAndPerturb(originVar, coarsenVar):
    perturbVar = originVar - coarsenVar
    coarsenVarAnamoly = coarsenVar - np.mean(originVar, axis=(1, 2), keepdims=True)
    return coarsenVarAnamoly, perturbVar
...
def runningMean(data, windowLength):
    ...
    for i in range(meanData.shape[0]):
        meanData[i] = np.mean(data[i:i+windowLength], axis=0)
    return meanData
